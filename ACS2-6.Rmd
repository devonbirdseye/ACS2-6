...
```
```{r}
test_ttest <- function(csv){
  result <- NULL
  for(i in 1:(nrow(csv))){
  mo17v = (as.numeric(csv[i, 7:8]))
  b73v = (as.numeric(csv[i, 2:3]))
  parentv = rowMeans(cbind(mo17v, b73v))
  hybv = (as.numeric(csv[i, 5:6]))
  t_test <- t.test(parentv, hybv, var.equal = TRUE)
  pval <- t_test$p.value
  result[i] <- pval
  }
...
```
```{r}
colnames(df_MergeAcc_NoDupes_merge_fixed_NoDecoys)
```
```{r}
#Build distance matrix of bio reps
df_MergeAcc_NoDupes_dist_bioreps <- df_MergeAcc_NoDupes_merge_fixed_NoDecoys[,2:11]

colnames(df_MergeAcc_NoDupes_dist_bioreps)<- c("b73.1", "b73.2", "b73.3", "BM.1", "BM.2", "mo17.1", "mo17.2", "acs.1", "acs.2", "acs.3")
...
res.dist_bioreps <- factoextra::get_dist(df_MergeAcc_NoDupes_dist_bioreps, stand = TRUE, method = "pearson")
...
UID_ACS_reorder_ForDeseq <- UID_reorder_OneDupe[,(c(16:18,9:11))]
colnames(UID_ACS_reorder_ForDeseq) <- c("ACS.1", "ACS.2", "ACS.3", "B73.1", "B73.2", "B73.3")
rownames(UID_ACS_reorder_ForDeseq) <- UID_reorder_OneDupe$UID

#Create colData object for Deseq2:
colData_ACS_DF <- data.frame("condition"= c("ACS","ACS","ACS", "B73", "B73", "B73"), stringsAsFactors = T)
...
```
```{r}
df_DF[grep("Zm00001d020686",df_DF$ID),]
```
```{r}
3988771/(mean(7971387, 3659407))
4967139/7971387
```
...
```{r}
#Re-structure for input in Deseq2:
UID_reorder_OneDupe$MP1 <- rowMeans(UID_reorder_OneDupe[,c(9,12)])
UID_reorder_OneDupe$MP2 <- rowMeans(UID_reorder_OneDupe[,c(10,13)])
...
```{r}
FoldChanges_HYB_DF <- res_HYB_DF$log2FoldChange
names(FoldChanges_HYB_DF) <- rownames(res_HYB_DF)
head(FoldChanges_HYB_DF)
```
...
```
```{r}
countData_ACS_DF_Log2FC$acs2B73.1 <- log2(countData_ACS_DF_Log2FC$ACS.1/countData_ACS_DF_Log2FC$B73.1)
...
```
```{r}
countData_ACS_DF_Log2FC <- countData_ACS_DF_Log2FC[,c(7:9)]
```
...
keggres_ACS_DF <- gage(FoldChanges_ACS_DF, gsets = zma$kg.sets)
keggres_HYB_DF <- gage(FoldChanges_HYB_DF, gsets = zma$kg.sets)
```
```{r}
keggres_HYB_DF2 <- gage(countData_HYB_DF_Log2FC, gsets = zma$kg.sets, samp = c(1:2))
keggres_ACS_DF2 <- gage(countData_ACS_DF_Log2FC, gsets = zma$kg.sets, samp = c(1:3))
```
```{r}
keggres_HYB_DF2_NotSameDir <- gage(countData_HYB_DF_Log2FC, gsets = zma$kg.sets, samp = c(1:2), same.dir = F)
keggres_ACS_DF2_NotSameDir <- gage(countData_ACS_DF_Log2FC, gsets = zma$kg.sets, samp = c(1:3), same.dir = F)
```
```{r}
keggres_ACS_DF_NotSameDir <- gage(FoldChanges_ACS_DF, gsets = zma$kg.sets, same.dir = F)
keggres_HYB_DF_NotSameDir <- gage(FoldChanges_HYB_DF, gsets = zma$kg.sets, same.dir = F)
```
...
```{r}
PVfun(keggres_HYB_DF2_NotSameDir$greater, countData_HYB_DF_Log2FC)
```
