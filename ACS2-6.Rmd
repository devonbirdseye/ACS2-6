...
PrevSL <- read.csv("CSVs/PreviousAnalysis/Seedling Leaf TMT10 030419.csv", stringsAsFactors = F)
#trim and reorder columns
PrevSL_trim <- PrevSL[,c(16,17,2:10)]
colnames(PrevSL_trim) <- c("Accession", "Protein", "b731", "b732", "b733", "mo171", "mo172", "mo173", "bm1", "bm2", "bm3")
```

## Define Biomart function for converting accessions
...
## Accession Conversion for acs data
### Biomart
...
#get GI accessions
giAccessions<-df_NoDecoys$Accession
...
print(paste0("Number of Missing V4 accessions left as GI in V4 column = ",
nrow(df_NoDecoys_Biomart_epost[-(grep("Zm0", df_NoDecoys_Biomart_epost$V4)),])))
```


## Accession Conversion for Previous Seedling Leaf data
### Biomart
```{r Create vector of RefSeq Peptide IDs, eval=FALSE, echo=FALSE}
#remove "decoy hits"
PrevSL_trim_NoDecoys <- PrevSL_trim[-grep("decoy",PrevSL_trim$Protein),]
#substring to just accession
RefSeqPepV <- substr(PrevSL_trim_NoDecoys$Protein, start = 5, stop = 18)
#Remove any letters
RefSeqPepV2 <- gsub("- [A-z]", "", RefSeqPepV)
#Make accession vector a new column on PrevSL_trim_NoDecoys
PrevSL_trim_NoDecoys$RefSeqPep <- RefSeqPepV2
```
```{r Convert accessions via biomart, eval=FALSE, echo=FALSE}
#Run Biomart pull function
AccConv <- BiomartRefSeqPepFun(PrevSL_trim_NoDecoys$RefSeqPep)
...
```
```{r Merge bm accessions back to df, eval=FALSE, echo=FALSE}
PrevSL_trim_NoDecoys_Biomart <- merge.data.frame(PrevSL_trim_NoDecoys, AccConv, by.x = "RefSeqPep", by.y = "refseq_peptide", all=T)
```
### epost/efetch
```{r Create GI vector, eval=FALSE, echo=FALSE}
###Use the epost/efetch functions written by Laura to pull maizegdb accessions from xml as well
...
#get GI accessions
giAccessions<-PrevSL_trim_NoDecoys$Accession
#remove gi accessions with alphas AKA nonconvertible
giAccessions<-giAccessions[-grep(giAccessions, pattern = "[a-zA-Z]")]
```
```{r epost, efetch, eval=FALSE, echo=FALSE}
#if re-running this, delete old copy of efetch_PrevSL.txt from file first
#first, convert GI to UID
#because more than 500, need to use epost first
p <- epost(giAccessions, db = "protein")
#efetch genpept file format for each UID, save to text file (requirement for epost>500)
efetch(p, db="protein",retmode = "text", rettype="gp", outfile = "efetch_PrevSL.txt")
```
```{r UID vec, eval=FALSE, echo=FALSE}
##Pause a couple seconds while the efetch_PrevSL.txt file synchs to cloud, then continue:

#read in UID text
UIDVec<-read.delim(file="efetch_PrevSL.txt", header = FALSE)
#convert to vector
UIDVec<-UIDVec[,1]
#get gene IDs from full flat file
UIDVec2<-UIDVec[grep(UIDVec, pattern="GeneID")]
...
PrevSL_trim_NoDecoys_Biomart_epost <- merge.data.frame(PrevSL_trim_NoDecoys_Biomart, NCBIConvert2, by.x = "Accession", by.y = "GI", all=T)

##make new column for complete V4 accessions
#choose the epost ID if available
PrevSL_trim_NoDecoys_Biomart_epost$V4 <- PrevSL_trim_NoDecoys_Biomart_epost$maizegdb.epost
#if no epost ID, choose the 1st biomart ID
PrevSL_trim_NoDecoys_Biomart_epost$V4[is.na(PrevSL_trim_NoDecoys_Biomart_epost$V4)]<- PrevSL_trim_NoDecoys_Biomart_epost$maizegdb.bm.1[is.na(PrevSL_trim_NoDecoys_Biomart_epost$V4)]
#if no epost or biomart ID, choose the GI
PrevSL_trim_NoDecoys_Biomart_epost$V4[is.na(PrevSL_trim_NoDecoys_Biomart_epost$V4)]<- PrevSL_trim_NoDecoys_Biomart_epost$Accession[is.na(PrevSL_trim_NoDecoys_Biomart_epost$V4)]

print(paste0("Number of Missing V4 accessions left as GI in V4 column = ",
nrow(PrevSL_trim_NoDecoys_Biomart_epost[-(grep("Zm0", PrevSL_trim_NoDecoys_Biomart_epost$V4)),])))
```

```{r Save DF RDS, eval=FALSE, echo=FALSE}
saveRDS(df_NoDecoys_Biomart_epost, "df_NoDecoys_Biomart_epost")
saveRDS(PrevSL_trim_NoDecoys_Biomart_epost, "PrevSL_trim_NoDecoys_Biomart_epost")
```


## Re-start here after cleared environment - Data import and editing
### Read DF RDS
```{r Read DF RDS}
df_NoDecoys_Biomart_epost <- readRDS("df_NoDecoys_Biomart_epost")
PrevSL_trim_NoDecoys_Biomart_epost <- readRDS("PrevSL_trim_NoDecoys_Biomart_epost")
```
```{r}
PrevSL_trim_NoDecoys_Biomart_epost[(PrevSL_trim_NoDecoys_Biomart_epost$maizegdb.bm.1==PrevSL_trim_NoDecoys_Biomart_epost$maizegdb.epost),]
```
...
DF_PrevSL <- PrevSL_trim_NoDecoys_Biomart_epost
DF_PrevSL <- DF_PrevSL[,c(1,2,27,29,3,4:12)]
colnames(DF_PrevSL) <- c("GI", "RefSeqPep", "UID", "V4", "Protein", "b731", "b732", "b733", "mo171", "mo172", "mo173", "bm1", "bm2", "bm3")
```
...
```
```{r}
DF$avg.bm.mp <- GeomeanSetCols(DF, 17:18)
DF$avg.acs.b73 <- GeomeanSetCols(DF, 19:21)
...
#Set NAs to 0
DF_PrevSL[DF_PrevSL==0]<-NA
# calc. mp values
DF_PrevSL$mp1 <- rowMeans(DF_PrevSL[,c(6,9)], na.rm = T)
DF_PrevSL$mp2 <- rowMeans(DF_PrevSL[,c(7,10)], na.rm = T)
DF_PrevSL$mp3 <- rowMeans(DF_PrevSL[,c(8,11)], na.rm = T)
#calc. ratios
DF_PrevSL$bm1.mp1 <- DF_PrevSL$bm1/DF_PrevSL$mp1
DF_PrevSL$bm2.mp2 <- DF_PrevSL$bm2/DF_PrevSL$mp2
DF_PrevSL$bm3.mp3 <- DF_PrevSL$bm3/DF_PrevSL$mp3
```
```{r}
#calc average and standard dev
DF_PrevSL$avg.bm.mp <- GeomeanSetCols(DF_PrevSL, 18:20)
DF_PrevSL$sds.bm.mp <- rowSds(as.matrix(DF_PrevSL[,18:20]), na.rm = T)
```

### write l2fc tables for input in CornCyc
```{r write table of l2fc for CornCyc analyses, eval=FALSE, echo=FALSE}
write.table(na.omit(DF_PrevSL[,c(4,21)]), "Prot_SL.txt", sep="\t", row.names = F, col.names = F)
write.table(na.omit(DF[,c(4,32)]), "Prot_ACS.txt", sep="\t", row.names = F, col.names = F)
```
...
DF_PrevSL.bm.sig <- DF_PrevSL[(DF_PrevSL$sds.bm.mp<0.1),]
#subset to >10% and <10%
DF.acs.sig.10above <- DF.acs.sig[(DF.acs.sig$avg.acs.b73>1.1),]
DF.bm.sig.10above <- DF.bm.sig[(DF.bm.sig$avg.bm.mp>1.1),]
DF_PrevSL.bm.sig.10above <- DF_PrevSL.bm.sig[(DF_PrevSL.bm.sig$avg.bm.mp>1.1),]

DF.acs.sig.10below <- DF.acs.sig[(DF.acs.sig$avg.acs.b73<0.9),]
DF.bm.sig.10below <- DF.bm.sig[(DF.bm.sig$avg.bm.mp<0.9),]
DF_PrevSL.bm.sig.10below <- DF_PrevSL.bm.sig[(DF_PrevSL.bm.sig$avg.bm.mp<0.9),]

#Remove the one row of NA - dont know where this came from (It doesnt exist in DF or DF_SL)
DF.acs.sig.10above <- DF.acs.sig.10above[!(is.na(DF.acs.sig.10above$GI)),]
DF.bm.sig.10above <- DF.bm.sig.10above[!(is.na(DF.bm.sig.10above$GI)),]
DF_PrevSL.bm.sig.10above <- DF_PrevSL.bm.sig.10above[!(is.na(DF_PrevSL.bm.sig.10above$GI)),]
DF.acs.sig.10below <- DF.acs.sig.10below[!(is.na(DF.acs.sig.10below$GI)),]
DF.bm.sig.10below <- DF.bm.sig.10below[!(is.na(DF.bm.sig.10below$GI)),]
DF_PrevSL.bm.sig.10below <- DF_PrevSL.bm.sig.10below[!(is.na(DF_PrevSL.bm.sig.10below$GI)),]
```
...
```
```{r}
DF_PrevSL$bm1.mp1.l2fc <- log2(DF_PrevSL$bm1.mp1)
DF_PrevSL$bm2.mp2.l2fc <- log2(DF_PrevSL$bm2.mp2)
DF_PrevSL$bm3.mp3.l2fc <- log2(DF_PrevSL$bm3.mp3)
DF_PrevSL$avg.bm.mp.l2fc <- log2(DF_PrevSL$avg.bm.mp)
```
...
DF_PrevSL_NoDupUID <- DF_PrevSL[!(duplicated(DF_PrevSL$UID) | is.na(DF_PrevSL$UID)),]

nrow(DF_PrevSL)
nrow(DF_PrevSL_NoDupUID)
```
### subset to hyb and acs l2fc DFs for KEGG
```{r}
colnames(DF_NoDupUID)
```

```{r subset to hyb and acs l2fc DFs for KEGG}
DF.bm.l2fc <- DF_NoDupUID[,26:27]
DF.acs.l2fc <- DF_NoDupUID[,29:31]
DF.bm.AND.acs.l2fc <- DF_NoDupUID[,c(26:27,29:31)]
...
DF_PrevSL.bm.l2fc <- DF_PrevSL_NoDupUID[,24:26]
rownames(DF_PrevSL.bm.l2fc) <- DF_PrevSL_NoDupUID$UID
```
...
keggres_HYB_DF <- gage(DF.bm.l2fc, gsets = zma$kg.sets, samp = c(1:2))
keggres_ACS_DF <- gage(DF.acs.l2fc, gsets = zma$kg.sets, samp = c(1:3))
keggres_HYB.AND.ACS_DF <- gage(DF.bm.AND.acs.l2fc, gsets = zma$kg.sets, samp = c(1:5))
keggres_PrevSL_DF <- gage(DF_PrevSL.bm.l2fc, gsets = zma$kg.sets, samp = c(1:3))
```
...
PVfun(keggres_PrevSL_DF$greater, DF_PrevSL.bm.l2fc)
PVfun(keggres_PrevSL_DF$less, DF_PrevSL.bm.l2fc)
...
PVfun(keggres_HYB.AND.ACS_DF$greater, DF.bm.AND.acs.l2fc)
...
BiosynthesisAAs <- pathview(gene.data = DF.acs.l2fc, pathway.id = "zma01230", species="zma", out.suffix = "BiosynthesisAAs_ACS", keys.align = "y", kegg.native = F, match.data = F, multi.state = T, same.layer = T, cex=0.125, afactor=3, map.null = T, expand.node = T)
```
...
GamerCut_PrevSL <- gamer[(gamer$db_object_id %in% DF_PrevSL$V4),]
```
...
```{r}
DF.ribosomal <- DF[(grep("ribosomal", DF$Protein)),]
DF.ribosomal <- DF.ribosomal[-(grep("NP_04", DF.ribosomal$Protein)),]
DF_PrevSL.ribosomal <- DF_PrevSL[(grep("ribosomal", DF_PrevSL$Protein)),]
DF_PrevSL.ribosomal <- DF_PrevSL.ribosomal[-(grep("NP_04", DF_PrevSL.ribosomal$Protein)),]

DF.GO.photo <- DF[(DF$V4%in%go.photosynthesis$db_object_id),]
DF_PrevSL.GO.photo <- DF_PrevSL[(DF_PrevSL$V4%in%go.photosynthesis$db_object_id),]

DF.PE <- DF[(grep("NP_04", DF$Protein)),]
DF_PrevSL.PE <- DF_PrevSL[(grep("NP_04", DF_PrevSL$Protein)),]

DF.PE.ribo <- DF.PE[(grep("ribosomal", DF.PE$Protein)),]
DF_PrevSL.PE.ribo <- DF_PrevSL.PE[(grep("ribosomal", DF_PrevSL.PE$Protein)),]

DF.PE <- DF.PE[-(grep("ribosomal", DF.PE$Protein)),]
DF_PrevSL.PE <- DF_PrevSL.PE[-(grep("ribosomal", DF_PrevSL.PE$Protein)),]
```
```{r}
DF.all.cut <- DF[,c(5,23)]
DF_PrevSL.all.cut <- DF_PrevSL[,c(5,21)]
DF.all.cut.DF_PrevSL.all.cut <- merge.data.frame(DF.all.cut, DF_PrevSL.all.cut, by="Protein", all=F)
DF.all.cut.DF_PrevSL.all.cut$ID <- "all"
```
```{r}
DF.ribosomal.cut <- DF.ribosomal[,c(5,23)]
DF_PrevSL.ribosomal.cut <- DF_PrevSL.ribosomal[,c(5,21)]
DF.ribosomal.cut.DF_PrevSL.ribosomal.cut <- merge.data.frame(DF.ribosomal.cut, DF_PrevSL.ribosomal.cut, by="Protein", all=F)
DF.ribosomal.cut.DF_PrevSL.ribosomal.cut$ID <- "ribosomal"

DF.ribosomal.cut.acs.bm <- DF.ribosomal[,c(5,22,23)]
DF.ribosomal.cut.acs.bm$ID <- "ribosomal"
```
```{r}
DF.GO.photo.cut <- DF.GO.photo[,c(5,23)]
DF_PrevSL.GO.photo.cut <- DF_PrevSL.GO.photo[,c(5,21)]
DF.GO.photo.cut.DF_PrevSL.GO.photo.cut <- merge.data.frame(DF.GO.photo.cut, DF_PrevSL.GO.photo.cut, by="Protein", all=F)
DF.GO.photo.cut.DF_PrevSL.GO.photo.cut$ID <- "GO.photo"

DF.GO.photo.cut.acs.bm <- DF.GO.photo[,c(5,22,23)]
DF.GO.photo.cut.acs.bm$ID <- "GO.photo"
```
```{r}
write.csv(DF_PrevSL.PE, "DF_PrevSL.PE.csv")
```

```{r}
DF.PE.cut <- DF.PE[,c(5,23)]
DF_PrevSL.PE.cut <- DF_PrevSL.PE[,c(5,21)]
DF.PE.cut.DF_PrevSL.PE.cut <- merge.data.frame(DF.PE.cut, DF_PrevSL.PE.cut, by="Protein", all=F)
DF.PE.cut.DF_PrevSL.PE.cut$ID <- "PE"

DF.PE.cut.acs.bm <- DF.PE[,c(5,22,23)]
DF.PE.cut.acs.bm$ID <- "PE"
```
```{r}
DF.PE.ribo.cut <- DF.PE.ribo[,c(5,23)]
DF.PE.ribo.cut.bm <- DF.PE.ribo[,c(5,22)]
DF_PrevSL.PE.ribo.cut <- DF_PrevSL.PE.ribo[,c(5,21)]
DF.PE.ribo.cut.DF_PrevSL.PE.ribo.cut <- merge.data.frame(DF.PE.ribo.cut, DF_PrevSL.PE.ribo.cut, by="Protein", all=F)
DF.PE.ribo.cut.DF_PrevSL.PE.ribo.cut$ID <- "PE.ribo"

DF.PE.ribo.cut.acs.bm <- DF.PE.ribo[,c(5,22,23)]
DF.PE.ribo.cut.acs.bm$ID <- "PE.ribo"
```
```{r}
DF.cut.DF_PrevSL.cut.rbind <- rbind(DF.all.cut.DF_PrevSL.all.cut, DF.ribosomal.cut.DF_PrevSL.ribosomal.cut, DF.GO.photo.cut.DF_PrevSL.GO.photo.cut, DF.PE.cut.DF_PrevSL.PE.cut, DF.PE.ribo.cut.DF_PrevSL.PE.ribo.cut)
```
```{r}
DF.cut.DF_PrevSL.cut.rbind.NE <- rbind(DF.all.cut.DF_PrevSL.all.cut, DF.ribosomal.cut.DF_PrevSL.ribosomal.cut, DF.GO.photo.cut.DF_PrevSL.GO.photo.cut)
DF.cut.DF_PrevSL.cut.rbind.PE <- rbind(DF.all.cut.DF_PrevSL.all.cut, DF.PE.cut.DF_PrevSL.PE.cut, DF.PE.ribo.cut.DF_PrevSL.PE.ribo.cut)
```
```{r}
DF.cut.acs.bm.PE.rbind <- rbind(DF.PE.cut.acs.bm,DF.PE.ribo.cut.acs.bm)
DF.cut.acs.bm.NE.rbind <- rbind(DF.GO.photo.cut.acs.bm, DF.ribosomal.cut.acs.bm)
```
```{r}
write.csv(DF.cut.DF_PrevSL.cut.rbind, "DF.cut.DF_PrevSL.cut.rbind.csv")
```
```{r}
DF.cut.DF_PrevSL.cut.rbind.NE.lims <- DF.cut.DF_PrevSL.cut.rbind.NE
DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.acs.b73 <- log2(DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.acs.b73)
DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.bm.mp <- log2(DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.bm.mp)
DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.acs.b73[DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.acs.b73>1.0]<- 1.0
DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.acs.b73[DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.acs.b73< -1.0]<- -1.0
DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.bm.mp[DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.bm.mp>1.0]<- 1.0
DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.bm.mp[DF.cut.DF_PrevSL.cut.rbind.NE.lims$avg.bm.mp< -1.0]<- -1.0
```
```{r}
DF.cut.DF_PrevSL.cut.rbind.PE.lims <- DF.cut.DF_PrevSL.cut.rbind.PE
DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.acs.b73 <- log2(DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.acs.b73)
DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.bm.mp <- log2(DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.bm.mp)
DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.acs.b73[DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.acs.b73>1.0]<- 1.0
DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.acs.b73[DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.acs.b73< -1.0]<- -1.0
DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.bm.mp[DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.bm.mp>1.0]<- 1.0
DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.bm.mp[DF.cut.DF_PrevSL.cut.rbind.PE.lims$avg.bm.mp< -1.0]<- -1.0
```
```{r}
DF.cut.acs.bm.PE.rbind.lims <- DF.cut.acs.bm.PE.rbind
DF.cut.acs.bm.PE.rbind.lims$avg.acs.b73 <- log2(DF.cut.acs.bm.PE.rbind.lims$avg.acs.b73)
DF.cut.acs.bm.PE.rbind.lims$avg.bm.mp <- log2(DF.cut.acs.bm.PE.rbind.lims$avg.bm.mp)
DF.cut.acs.bm.PE.rbind.lims$avg.acs.b73[DF.cut.acs.bm.PE.rbind.lims$avg.acs.b73>1.0]<- 1.0
DF.cut.acs.bm.PE.rbind.lims$avg.acs.b73[DF.cut.acs.bm.PE.rbind.lims$avg.acs.b73< -1.0]<- -1.0
DF.cut.acs.bm.PE.rbind.lims$avg.bm.mp[DF.cut.acs.bm.PE.rbind.lims$avg.bm.mp>1.0]<- 1.0
DF.cut.acs.bm.PE.rbind.lims$avg.bm.mp[DF.cut.acs.bm.PE.rbind.lims$avg.bm.mp< -1.0]<- -1.0
```
```{r}
write.csv(DF.cut.acs.bm.PE.rbind,"DF.cut.acs.bm.PE.rbind.csv")
```

```{r}
DF.cut.acs.bm.NE.rbind.lims <- DF.cut.acs.bm.NE.rbind
DF.cut.acs.bm.NE.rbind.lims$avg.acs.b73 <- log2(DF.cut.acs.bm.NE.rbind.lims$avg.acs.b73)
DF.cut.acs.bm.NE.rbind.lims$avg.bm.mp <- log2(DF.cut.acs.bm.NE.rbind.lims$avg.bm.mp)
DF.cut.acs.bm.NE.rbind.lims$avg.acs.b73[DF.cut.acs.bm.NE.rbind.lims$avg.acs.b73>1.0]<- 1.0
DF.cut.acs.bm.NE.rbind.lims$avg.acs.b73[DF.cut.acs.bm.NE.rbind.lims$avg.acs.b73< -1.0]<- -1.0
DF.cut.acs.bm.NE.rbind.lims$avg.bm.mp[DF.cut.acs.bm.NE.rbind.lims$avg.bm.mp>1.0]<- 1.0
DF.cut.acs.bm.NE.rbind.lims$avg.bm.mp[DF.cut.acs.bm.NE.rbind.lims$avg.bm.mp< -1.0]<- -1.0
```
```{r}
ggplot(DF.cut.DF_PrevSL.cut.rbind.PE.lims, mapping = aes(x=(avg.acs.b73), y=(avg.bm.mp), color=ID))+
  geom_point(size=.5)+
  theme_bw()+
 ...
 # geom_abline(slope = 1, color="grey", size=.5)+
  geom_vline(color="grey", size=.5, xintercept = 0)+
  geom_hline(color="grey", size=.5, yintercept = 0)+
  coord_fixed(expand = F)+
  xlim(-1,1)+
  ylim(-1,1)+
  ggtitle("Shared plastid-encoded proteins")
ggsave("Scatter_SharedPE.png")
```


```{r}
ggplot(DF.cut.DF_PrevSL.cut.rbind.NE.lims, mapping = aes(x=(avg.acs.b73), y=(avg.bm.mp), color=ID))+
  geom_point(size=.5)+
  theme_bw()+
  scale_color_manual(values=c("grey80","blue", "red"))+
  ylab("log2(BxM/MP)")+
 ...
 # geom_abline(slope = 1, color="grey", size=.5)+
  ...
  xlim(-1,1)+
  ylim(-1,1)+
  ggtitle("Shared nuclear-encoded proteins")
ggsave("Scatter_SharedNE.png")
```
```{r}
ggplot(DF.cut.acs.bm.PE.rbind, mapping = aes(x=(avg.acs.b73), y=(avg.bm.mp), color=ID))+
  geom_point(size=.5)+
  theme_bw()+
  scale_color_manual(values=c("grey80","blue", "red"))+
  ylab("log2(BxM/MP)")+
