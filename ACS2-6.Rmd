...
---
#####Start at line 192 if environment is cleared
...
PrevSL_trim <- PrevSL
PrevSL_trim <- PrevSL_trim[,c(16,17,2:10)]
...
```{r Merge bm accessions back to df}
...
```{r Merge Acc Conversion back with Data}
...
```{r Merge Acc Conversion back with PrevSL_trim Data}
...
```{r Save df_NoDecoys_Biomart_epost RDS}
...
```{r Read df_NoDecoys_Biomart_epost RDS as DF}
DF <- readRDS("df_NoDecoys_Biomart_epost")
...
```{r write table PrevSL l2fc for Corn Cyc analyses}
...
```{r Save l2fc data for CornCyc analysis}
...
```{r Make .png images of KEGG pathway enrichment dataframes}
...
```
```{r}
CysMetMetabolism_HYB <- pathview(PrevSL_trim_epost2.bm.l2fc, pathway.id = "zma00270", species="zma", out.suffix = "CysMetMetabolism_PrevSL")
```
```{r}
...
```
```{r}
keggview.graph(plot.data.gene = DF.acs.l2fc, path.graph = "zma01230", species="zma", out.suffix = "BiosynthesisAAs_ACS.kvn", node.data = AAxml, is.signal = F)
```
...
```{r GO analysis venn diagram sections}
#above
...
```
```{r}
colnames(df_NoDecoys_Biomart_epost)
```
...
```
```{r}
GO.DF.l$DF.bm.sig.10above.godataBP
```

```{r}
print(showGroupDensity(DF.bm.sig.10above.godataBP, "GO:0009765", ranks = T, rm.one = F))
```
